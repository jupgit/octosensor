...
///     C E N T R A L   ( 2026 )
///
///  This central OVERRIDES its Wi-Fi STA MAC address so that the
///  nodes (which unicast to 94:B9:7E:C2:CA:D0) are received here
///  without touching the node firmware.
///
///  ESP-NOW delivers a unicast frame only to the interface whose
///  MAC matches the destination, so we spoof that MAC at boot.
///
...
// ************************************************************
// This is the MAC the NODES are transmitting TO. We force this
// board's STA interface to use it so their messages arrive here.
// (Must be a unicast address: first byte's low bit = 0. 0x94 is OK.)
uint8_t centralMAC[] = {0x94, 0xB9, 0x7E, 0xC2, 0xCA, 0xD0};
// ************************************************************
...
void setup() {
  ...
  // ***** OVERRIDE THE STA MAC ADDRESS *****
  // Force this board to use the MAC the nodes transmit to, so their
  // unicast ESP-NOW messages are delivered to this interface.
  esp_wifi_set_mac(WIFI_IF_STA, &centralMAC[0]);

  Serial.print("Central MAC now: ");
  Serial.println(WiFi.macAddress());   // should read 94:B9:7E:C2:CA:D0
